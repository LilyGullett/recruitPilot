...
#Trying to determine the number of errors per rack per timepoint, in the code below it is for the taglab segmentation vs the trained eye absolute value
rackErr <- plugData1 %>% 
  group_by(timepoint, rack) %>%
  summarise(errorPerRack = sum(trainedVsTaglabAbs))
rackErr
...
tVu <- plugData1 %>%
   select(timepoint, rack, plug, color, trainedEyeCount, untrainedEyeCount) %>%
   pivot_longer(cols = trainedEyeCount:untrainedEyeCount,
                names_to = "type",
                values_to = "counts") %>%
  mutate_at(c('type'), as.factor)

t.test(counts ~ type, data = tVu)
