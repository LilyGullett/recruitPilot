...
# you need to change your xlab and ylab labels and title below and then add the theme statements from previous graphs and other graph (in the efficiency script)

comparingPlot <- ggplot(plugData1, aes(x = timepoint, y = count)) + geom_bar(position = 'dodge', stat = 'identity', aes(fill = type))+
  facet_grid( scales = 'free_x')+
  xlab("Timepoint (weeks)") + ylab("Average Number of Recruits per ???") +
    ggtitle("Comparing Inventory Counts") +
    theme_bw()
