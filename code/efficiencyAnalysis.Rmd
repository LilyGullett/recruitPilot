...
timeData1 <- timeData %>% 
  select(timepoint, rack, trainedEyeTime, untrainedEyeTime) %>% 
  pivot_longer(cols = trainedEyeTime:untrainedEyeTime,
               names_to = 'type', 
...
comparingPlot <- ggplot(timeData1, aes(x = timepoint, y = inventoryTime)) + geom_bar(position = 'dodge', stat = 'identity', aes(fill = type))+
  facet_grid(.~rack, scales = 'free')+ xlab("Timepoint (weeks)") + ylab("Time Taken (minutes)") +
    ggtitle("Trained Eye Inventory Time vs Untrained") +
    theme_bw()
...
ggsave("../figures/trainedVsUntrainedTime.png", plot = comparingPlot, width = 15, height = 10, units = "in", dpi = 600)
